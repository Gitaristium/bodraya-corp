...
function fonts() {
  return src([
    ...
    .pipe(dest('dist/assets/webfonts'))  //для разработки локально
    .pipe(browserSync.stream())       //для разработки локально
  // .pipe(dest('preFTP'))            //для разработки на хосте
}
...
exports.default = series(parallel(html, styles, scripts, fonts, images), parallel(browsersync, watching));  //для разработки локально
